...
class TaskAdapter(
    ...
    override fun onBindViewHolder(holder: TaskViewHolder, position: Int) {
        ...
        holder.bind(task)
        //Bind data to ViewHolder (You can run app to check)
        when {
            //Display title based on status using TitleTextView
            task.isCompleted -> {
                //DONE
    ...
    inner class TaskViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(task: Task) {
            ...
            tvTitle.state = when {
                task.isCompleted -> TaskTitleView.DONE
                task.dueDateMillis < System.currentTimeMillis() -> TaskTitleView.OVERDUE
                else -> TaskTitleView.NORMAL
            }
        }
